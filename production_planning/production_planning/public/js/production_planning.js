...
// Entry file expected by Frappe esbuild bundler.
// It simply includes the existing Planning Sheet client script.
...
frappe.ui.form.on('Planning Sheet', {
    refresh: function (frm) {
        ...
        if (frm.doc.docstatus === 1 && frm.doc.planning_status === "In Production") {
            frm.add_custom_button(__('Single Production Entry'), function () {
                frappe.confirm(__('Generate Single Production Entry for all Work Orders?'), function () {
                    frappe.call({
                        doc: frm.doc,
                        method: 'make_consolidated_production_entry',
                        callback: function (r) {
                            if (r.message) {
                                frappe.msgprint({
                                    title: __('Production Entries Created'),
                    ...
                    if (r.message) {
                        ...
                        if (r.message.items) {
                            ...
                            r.message.items.forEach(function (item) {
                                ...
                                row.so_item = item.name; // Crucial for traceability
