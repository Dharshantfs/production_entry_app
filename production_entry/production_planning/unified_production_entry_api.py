...
def _gsm_draft_sprs_for_session(run_date, shift, unit, extra_pp_ids=None) -> list[dict]:
	...
	# Same-date SPRs for session PPs even when shift/unit labels differ (desk-created runs).
	...
	# Drop empty stub SPRs when another SPR for the same PP already has rolls.
	...
	# Client sessionSprs last-write-wins — put the SPR with the most rolls last per PP.
