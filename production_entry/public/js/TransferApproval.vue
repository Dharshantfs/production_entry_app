<template>
  <div class="sequence-approval-container ta-transfer">
    <div class="dashboard-header">
      <div class="header-left">
        <h2>Transfer Approvals Dashboard</h2>
        <p class="text-muted">Review batches, reorder queue, approve to create draft Stock Entry (Party + Nature of Processing).</p>
      </div>
    ...
    <div v-if="loading" class="loading-state">
      ...
      <p class="mt-3">Loading transfers…</p>
    </div>

    <div v-else-if="!filteredList.length" class="empty-state">
      <div class="empty-icon">📦</div>
      <template v-if="!list.length">
        <h3>No transfer approvals yet</h3>
        <p>Submit transfers from Production Table or Logistics Kanban.</p>
      </template>
      <template v-else>
        <h3>No transfers match this filter</h3>
    ...
    <div v-else class="approval-layout">
      <div class="approval-sidebar">
        ...
        >
          <div class="card-title">
            <span class="plan-badge">{{ item.to_destination_label || item.name }}</span>
          </div>
      ...
      <div class="approval-editor" v-if="selected">
        <div class="editor-header">
          ...
          <div class="editor-actions" style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap">
            ...
            <div v-else-if="selected.status === 'Approved'" class="status-indicator">
        ...
        <div class="editor-info" v-if="canAct && isPending(selected.status)">
          ...
          <span>Drag rows to set batch queue order. On approve, draft STE gets Party = {{ selected.to_company }} and Nature of Processing above.</span>
        </div>

        <div v-if="selected.stock_entry" class="ta-ste-banner">
          Stock Entry:
        ...
        <div class="sequence-list" v-if="lines.length">
          ...
          <div class="draggable-container" ref="dragContainer">
            <div v-for="(ln, index) in lines" :key="ln.name" class="sequence-item" :data-id="ln.name">
              ...
              <div class="col-qty text-right font-weight-bold">{{ formatQty(ln.qty) }}</div>
            </div>
          </div>
        ...
        <p v-else class="ta-empty-lines">No lines on this transfer.</p>
      ...
      <div class="editor-placeholder" v-else>
        <div class="placeholder-content">
          ...
          <p>Select a transfer from the sidebar to review and approve.</p>
        </div>
      </div>
    </div>
  </div>
...
async function select(item) {
  ...
  detailNature.value = doc.nature_of_processing || item.nature_of_processing || "";
  await nextTick();
  initSortable();
}
...
async function saveLineOrder() {
  ...
  try {
    await frappe.call({
      method: `${API}.reorder_transfer_approval_lines`,
      args: {
...
function approve() {
  ...
  const party = selected.value.to_company;
  const nature = detailNature.value || selected.value.nature_of_processing || "—";
  frappe.confirm(
    __("Approve transfer to <b>{0}</b>?<br><br>Party on STE: <b>{1}</b><br>Nature of Processing: <b>{2}</b>", [
      selected.value.to_destination_label,
      party,
      nature,
    ]),
    async () => {
      ...
      try {
        ...
        frappe.show_alert({
          message: __("Approved — STE {0}", [r.message?.stock_entry || ""]),
          indicator: "green",
        });
        await loadList();
        const hit = list.value.find((x) => x.name === selected.value?.name);
...
function reject() {
  if (!selected.value) return;
  frappe.confirm(__("Reject this transfer request?"), async () => {
    busy.value = true;
    try {
      await frappe.call({
