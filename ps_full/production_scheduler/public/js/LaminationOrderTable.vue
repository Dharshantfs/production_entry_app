<template>
  <div class="cc-container">
    ...
    <div class="cc-table-container">
      <div class="cc-table-unit-header lot-header">Lamination Unit Ã¢â‚¬â€ Planned orders (104)</div>
      <table class="cc-prod-table lot-table">
        ...
        <tbody>
          ...
          >
            ...
            <td class="cell-center font-mono font-bold" style="font-size:11px;color:#047857;">{{ row.lamination_booking_id || "Ã¢â‚¬â€" }}</td>
            ...
            <td class="cell-center">{{ row.fabric_gsm || "Ã¢â‚¬â€" }}</td>
            <td class="cell-center">{{ row.lamination_gsm ?? row.gsm }}</td>
            <td class="cell-right">{{ row.planned_meter ?? "Ã¢â‚¬â€" }}</td>
            ...
            <td class="cell-center">
              <button v-if="row.pp_id" type="button" @click="openProductionPlanView(row.planningSheet, row.salesOrderItem, row.itemName, row.pp_id || '')" class="cc-pp-btn">Ã°Å¸â€œâ€¹ View</button>
            ...
            <td class="cell-center">
              <div class="pt-stock-cell">
                <div v-if="row.pp_id" class="pt-pill-row">
                  ...
                  <span v-else class="pt-pill pt-pill-muted">SPR: Ã¢â‚¬â€</span>
                ...
                <span v-else-if="row.pp_id && row.wo_terminal" class="pt-wo-closed-hint">Ã¢Å“â€¦ WO closed</span>
            ...
            <td class="cell-center">
              <span v-if="arrangementUnlocked" class="cc-drag-handle" title="Drag to reorder inside same date">Ã¢â€¹Â®Ã¢â€¹Â®</span>
              <span v-else class="cc-lock-hint" title="Unlock arrangement to reorder">Ã°Å¸â€â€™</span>
            </td>
          </tr>
...
function itemSprPrimaryButtonTitle(item) {
  if (!item?.spr_name) return "";
  if (Number(item.spr_docstatus) === 0) return "Draft SPR Ã¢â‚¬â€ continue recording rolls.";
  if (item.wo_terminal) return "WO terminal Ã¢â‚¬â€ review only.";
